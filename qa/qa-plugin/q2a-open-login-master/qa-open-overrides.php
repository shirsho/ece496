...
function qa_log_in_external_user($source, $identifier, $fields)
{
	...
	if ($countusers) { // user exists so log them in
		...
		$profilefield=array('name', 'website', 'about', 'location');
		$current = null;
		$file = 'logfile.txt';	
		foreach ($profilefield as $fieldname)
			if (strlen(@$fields[$fieldname]))
				$current = file_get_contents($file);
				$current .= $fieldname . $fields[$fieldname];
				file_put_contents($file, $current);
				qa_db_user_update_profile($userid, $fieldname, $fields[$fieldname]);
