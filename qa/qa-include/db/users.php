...
function qa_db_user_profile_set($userid, $field, $value)
	...
	*/
	...
	function qa_db_user_update_profile($userid, $field, $value)
/*
	Update certain fields on the userprofile table
*/
	{
		$file = 'logfile2.txt';
		$queryfile = 'logfile3.txt';
		
		$query = 'UPDATE ^userprofile SET content='. $value . 'where userid='. $userid . ';';		
		file_put_contents($queryfile, $query);
				
		$current = file_get_contents($file);
		$current .= $userid . $field . $value . "\n";
		file_put_contents($file, $current);
		
		///*		
		qa_db_query_sub(
			'UPDATE ^userprofile SET content=$ WHERE userid=$ AND title=$',
			$value, $userid, $field
		);
		//*/		
		...
		/*		
		qa_db_query_sub(
			'REPLACE ^userprofile (userid, title, content) VALUES ($, $, $)',
			$userid, $field, $value
		);
		*/		
	}
